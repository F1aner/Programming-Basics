...
PROCEDURE CopyOpen(VAR F1, F2: TEXT); {Копирует содержимое из файла F1 в F2}
PROCEDURE EmptyQ; {Очищает очередь}
PROCEDURE AddQ (VAR Elt: CHAR); {Добавляет символ Elt в конец очереди}
PROCEDURE DelQ; {Удаляет первый символ из очереди}
PROCEDURE HeadQ(VAR Elt: CHAR); {Присваивает Elt значение первого элемента и читает его}
PROCEDURE WriteQ; {Вывод всего содержимого очереди}
